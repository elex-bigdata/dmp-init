  ...
  public static Pair<Matrix, Vector> loadModel(Configuration conf, Path... modelPaths)
    ...
    for (Path modelPath : modelPaths) {
      for (Pair<Text, VectorWritable> row :
        ...
        rows.add(Pair.of(Integer.parseInt(row.getFirst().toString()), row.getSecond().get()));//由于模型文件中的key为text型，因此需要做转换
        numTopics = Math.max(numTopics, Integer.parseInt(row.getFirst().toString()));//由于模型文件中的key为text型，因此需要做转换
        if (numTerms < 0) {
